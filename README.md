...
This project is a **working Bedrock case study** that demonstrates:

- **AWS Bedrock Integration**: Real LLM calls with Claude 3 Haiku
- **Dual-Agent Architecture**: Bedrock-powered agent with keyword fallback
- **Tool Integration**: Mock environmental tools with proper routing
- **API Development**: FastAPI server with chat endpoints
- **Error Handling**: Graceful fallback when Bedrock is unavailable

**What you can show recruiters:**

- Built a **working environmental AI agent** using AWS Bedrock
- Implemented **dual-agent architecture** for reliability
- Created **REST API** with proper error handling
- Demonstrated **tool integration** with mock environmental data
- Applied **AWS best practices** for Bedrock integration

The agent can actually answer questions about air quality, trip emissions, and wildfire alerts using either Bedrock LLM calls or keyword matching as a fallback.
